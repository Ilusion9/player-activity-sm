...
public void OnMapEnd()
{
	...
	data.AddQuery("CREATE TEMPORARY TABLE players_activity_temp SELECT steamid, DATE_FORMAT(date, \"%Y-%m-01\") as month_date, sum(seconds) FROM players_activity WHERE date < CURRENT_DATE - INTERVAL 1 MONTH AND date > CURRENT_DATE - INTERVAL 1 YEAR GROUP BY steamid, month_date;");
